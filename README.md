...
プロジェクトルートで実行します（Mac / Windows 共通）。

```sh
docker compose up --build
```

`server/pyproject.toml` または `server/uv.lock` を変更した場合、または初回セットアップ時はバックエンドイメージを再ビルドしてください。
...
YOLOv8 推論用の PyTorch / torchvision は **CPU 版**（[PyTorch CPU index](https://download.pytorch.org/whl/cpu)）に統一しています。Mac / Windows 上の Docker では GPU（CUDA）を使いません。初回起動時は YOLO モデル（`yolov8n.pt`）のダウンロードで数十秒かかることがあります。
