...
| マップ                        | `GET /api/parking-lots`（実装済み）, `POST /api/reservations`（実装済み）, `GET /api/parking-statuses/{parking_lot_id}`（実装済み・移行予定）       | 駐輪場マーカーは DB の `parking_lots` から取得し、予約モーダルは `POST /api/reservations` に接続済み。ルート検索は外部 OSRM。通知 UI はクライアントのみ（将来 `GET /api/notifications` を検討可）。                                                                                                                   |
| 駐輪場一覧                      | `GET /api/parking-statuses/{parking_lot_id}`（実装済み）, `GET /api/parking-lots` / `GET /api/parking-lots/{parking_lot_id}`（実装済み） | 現状フロントは主に `GET /api/parking-statuses/{id}` のみ。駐輪場マスタ API はマップの DB 連携で利用。                                                                                                                                                  |
| 予約管理                       | `GET /api/reservations/me`, `POST /api/reservations`, `PATCH /api/reservations/{reservation_id}`（実装済み）                       | 予約管理画面は固定モックを削除し、`GET /api/reservations/me` と `PATCH /api/reservations/{reservation_id}` に接続済み。マップからの予約作成は `POST /api/reservations` に接続済み。 |
| 設定                         | `GET /auth/me`, `PATCH /auth/me`（実装済み・プロフィール拡張）                                                                              | 現状は localStorage のみ。表示名等を API 化する案。                                                                                                                                                                                 |
| ダッシュボード                    | `GET /api/dashboard/summary`（実装済み）                                                                                           | admin / dev が利用。                                                                                                                                                                                                    |
| ゲートカメラ                     | `POST /gate-camera/images`, `GET /gate-camera/detections/latest`, `POST /gate-camera/trip`（実装済み）                              | dev ロール想定。入退場検知。                                                                                                                                                                                                   |
| 俯瞰カメラ                      | `POST /overhead-camera/images`, `GET /overhead-camera/detections/latest`（実装済み）                                               | dev ロール想定。台数カウントで空き更新。                                                                                                                                                                                            |
...
※ 現行サーバーでは `/api/parking-statuses`, `/api/dashboard/summary`, `/gate-camera/*`, `/overhead-camera/*` に `Depends(get_current_user)` が **未適用** のため、認証なしで応答する。**本設計ではユーザー向け・管理画面向けの読み取り API には Bearer 必須とする**（実装は別 Issue）。
...
| `/api/reservations/{reservation_id}` | PATCH | **書き込み** | **書き込み** | `reservations` UPDATE | Bearer 必須。現行は `{ "status": "cancelled" }` のみ許可し、完了済み・キャンセル済みは 409。 |
...
| `/gate-camera/images` | POST | メモリのみ | **書き込み** | `camera_detections`（ゲートデバイス） | 推論後に DB INSERT。未認証→本設計では dev ＋ Bearer。 |
| `/gate-camera/detections/latest` | GET | メモリ | **読み取り** | `camera_detections`（ゲートデバイス）またはキャッシュ | ゲートデバイス別の直近 1 件。 |
| `/gate-camera/trip` | POST | — | **書き込み** | `parking_statuses` | `direction: in/out` で `available_spots` を ±1 更新。 |
| `/overhead-camera/images` | POST | メモリのみ | **書き込み** | `camera_detections`（俯瞰デバイス）、`parking_statuses` | 検出台数から `available_spots = total_spots - detected_count` を直接更新。 |
| `/overhead-camera/detections/latest` | GET | メモリ | **読み取り** | `camera_detections`（俯瞰デバイス）またはキャッシュ | 俯瞰デバイス別の直近 1 件。`available_spots` / `total_spots` を含む。 |
...
| **[実装済み]** プロフィール更新（表示名など） | 設定                        | `/auth/me`     | PATCH | **Header** Bearer 必須。**Body**（例、すべて任意で部分更新） `name` (string) 将来: `notification_enabled` (boolean) 等                  | **200** 更新後 `UserOut` 同等 JSON **401** / **403** / **422**                                                                                                                       |
...
| 概要                           | 必要な画面 | エンドポイント                                  | メソッド | リクエスト                                                            | レスポンス                              |
| ---------------------------- | ----- | ---------------------------------------- | ---- | ---------------------------------------------------------------- | ---------------------------------- |
| **[実装済み]** マスタ＋最新空きの合成一覧 | 駐輪場一覧 | `/api/parking-lots`                      | GET  | 「4. マップ」節の表と同様。 | **200** 配列。`parking_lots` に最新 `available_spots` を付与して返す |
...
フロントの予約管理画面は固定モックを削除し、`GET /api/reservations/me` のレスポンスを **予約中** ≒ `status` が `reserved` または `active`、**履歴** ≒ `completed` または `cancelled` としてフロントでフィルタする。永続化の対応関係は「2.10 エンドポイント別の永続化（マトリクス）」を参照。


| 概要                 | 必要な画面            | エンドポイント                              | メソッド  | リクエスト                                                                                                  | レスポンス                                                                                                                                                                                                 |
| ------------------ | ---------------- | ------------------------------------ | ----- | ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[実装済み]** 自分の予約一覧 | 予約管理             | `/api/reservations/me`               | GET   | **Header** Bearer 必須     | **200** 配列。各 item: `id`, `parking_lot_id`, `parking_lot_name`, `location`, `start_time`, `end_time`, `status`, `total_amount`, `created_at` |
| **[実装済み]** 予約作成    | マップ（予約モーダル）、予約管理 | `/api/reservations`                  | POST  | **Header** Bearer 必須。**Body** `parking_lot_id` (uuid, 必須) `start_time` (string ISO8601, 必須) `end_time` (string ISO8601, 必須) | **201** 作成された予約オブジェクト **400** 時間帯不正 **404** 駐輪場不存在 **401**                                                                                                                                                       |
| **[実装済み]** 予約キャンセル | 予約管理             | `/api/reservations/{reservation_id}` | PATCH | **Header** Bearer 必須。**Path** `reservation_id` (uuid)。**Body** `{ "status": "cancelled" }` のみ                   | **200** 更新後オブジェクト **404** 自分の予約でない/不存在 **409** 既に完了またはキャンセル済み。画面の「キャンセル」ボタンは本 API 接続先。**「地図で確認」**はクライアントルーティングのみでよい（専用 API は不要）                                                         |
...
| **[実装済み]** プロフィール更新   | 設定     | `/auth/me` | PATCH | 「3. ログイン」節と同じ | 「3. ログイン」節と同じ     |
...
| 概要                     | 必要な画面 | エンドポイント                          | メソッド | リクエスト                                                         | レスポンス                                                                                                                                                                                  |
| ---------------------- | ----- | -------------------------------- | ---- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[実装済み]** 画像アップロード   | ゲートカメラ | `/gate-camera/images`            | POST | **Body**: 生バイナリ（画像）。**Header** `Content-Type`: `image/jpeg` 等 | **200** JSON: `message` (string), `content_type` (string または null), `size_bytes` (int)。**400** 空ボディ: `Image payload is empty`                                                          |
| **[実装済み]** 最新検出結果取得    | ゲートカメラ | `/gate-camera/detections/latest` | GET  | なし                                                            | **200** JSON: `detected_count` (int), `boxes` (配列), `received_at` / `content_type` は string または null, `size_bytes` (int)。各 box は `x`, `y`, `width`, `height`, `label`, `score` (float) |
| **[実装済み]** 通過イベント記録    | ゲートカメラ | `/gate-camera/trip`              | POST | **Body** `{ "direction": "in" \| "out" }`                     | **200** `{ "message": "ok" }`。**404** デバイス未存在                                                                                                                                       |
...
| 概要                     | 必要な画面 | エンドポイント                               | メソッド | リクエスト                                                         | レスポンス                                                                                                                                                                                                 |
| ---------------------- | ----- | ------------------------------------- | ---- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[実装済み]** 画像アップロード   | 俯瞰カメラ | `/overhead-camera/images`             | POST | **Body**: 生バイナリ（画像）。**Header** `Content-Type`: `image/jpeg` 等 | **200** JSON: `message`, `detected_count` (int), `available_spots` (int または null), `total_spots` (int または null), `content_type`, `size_bytes`。**400** 空ボディ: `Image payload is empty`                    |
| **[実装済み]** 最新検出結果取得    | 俯瞰カメラ | `/overhead-camera/detections/latest`  | GET  | なし                                                            | **200** JSON: `detected_count`, `boxes`, `available_spots`, `total_spots`, `received_at`, `content_type`, `size_bytes`                                                                                  |


俯瞰カメラは検出台数から `available_spots = total_spots - detected_count` で `parking_statuses` を直接更新する。ゲートカメラ（trip ±1）と同一駐輪場の空き台数を共有するため、運用時はソースを1つに固定すること。
...
| 認証       | `/api/` の一部、`/gate-camera/*` `/overhead-camera/*` が未保護 | ユーザー向け GET は **Bearer 必須**、IoT POST は **機器認証**、カメラは **dev ロール** 等に整理 |
